...
class mainMfp{
  ...
  public function __construct(){
	...
	public static function defaultOptions(){
	    ...
	    "mfp_mod_option_wp_del" => "0", // Удаление лого и ссылок wp в админке
			"mfp_mod_option_wp_emoji" => "0", // Удаление emoji
	    ...
	    "mfp_mod_option_wp_widgets" => array(
			...
			"mfp_mod_option_footer_text1" => "", // Ссылка в футере
			"mfp_mod_option_footer_text2" => "", // Подпись к ссылке
		...
		if(get_option('mfp_mod_options') == null) {
			...
			update_option('mfp_mod_image_url', '');
			update_option('mfp_mod_logo_url', '');
			update_option("mfp_mod_options", $mfp_options);
		}
	}
	 ...
	 * Активация плагина
	 */
	public static function mfp_activation(){
		if(get_option('mfp_mod_options') == null) {
			self::defaultOptions();
		}
	}
	 ...
	 * Деактивация плагина
	 */
	public static function mfp_deactivation(){}
  ...
  public function mfpAddStyle() {
    wp_register_style('mfp-style', plugins_url('mfp-mod-wp/css/mfp-mod-wp.css?v='.MFP_VERSION.''));
  ...
  public function mfpSaveOptions(){
		$data = array("mfp_mod_option_link" =>
			array("rss" => (isset($_POST['mfp_out_link']['rss'])) ? $_POST['mfp_out_link']['rss'] : '0',
						...
						"rel_canonical" => (isset($_POST['mfp_out_link']['rel_canonical'])) ? $_POST['mfp_out_link']['rel_canonical'] : '0',
			...
			"mfp_mod_option_wp_emoji" => (isset($_POST['mfp_out_wp_emoji'])) ? $_POST['mfp_out_wp_emoji'] : '0',
	...
	/** Удаление emoji */
	public function mfpRemoveEmoji(){
		add_filter('tiny_mce_plugin1s', array(&$this, 'mfpRemoveEmojiTiny'));
		add_filter('wp_resource_hints', array(&$this, 'mfpRemoveEmojiDnsPref'), 10, 2);
	...
	public function mfpRemoveEmojiTiny($plugins){
		if ( is_array( $plugins ) ) {
	...
	public function mfpRemoveEmojiDnsPref($urls, $relation_type){
	...
	public function custom_login_css() {
			...
			echo '.login h1 a {background-image: url("'.get_option('mfp_mod_logo_url').'")!important;}';
			echo '</style>';
    ...
    /** Удаление контекстного меню справки */
		...
		/** Удаление emoji */
		if($this->mfp_options['mfp_mod_option_wp_emoji'] == '1') {
			add_action('init', array(&$this, 'mfpRemoveEmoji'));
		}
    ...
    /** Замена текста в подвале "Спасибо вам за творчество с WordPress" */
		...
		if($this->mfp_options['mfp_mod_option_link']['rel_canonical'] == '1') {
